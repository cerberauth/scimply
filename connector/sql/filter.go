...
func (s *translationState) quote(name string) string {
	if s.dialect == DialectMySQL {
...
func (s *translationState) colRef(col string) string {
	if s.tableAlias != "" {
		return s.quote(s.tableAlias) + "." + s.quote(col)
	}
	return s.quote(col)
}
...
func translateAttrExpr(e *resource.AttrExpression, s *translationState) (string, error) {
	...
	var ref string
	if mref.Table != "" {
		ref = s.quote(mref.Table) + "." + s.quote(mref.Column)
	} else {
...
func translateValuePathExpr(e *resource.ValuePathExpression, s *translationState) (string, error) {
	...
	var parentID string
	if s.tableAlias != "" {
		parentID = s.quote(s.tableAlias) + ".id"
	} else {
		parentID = "id"
	}
	...
	return fmt.Sprintf(
		...
		s.quote(subTable), s.quote(subAlias), s.quote(subAlias), parentID, inner,
	), nil
}
