...
import (
	...
	mysqlconn "github.com/cerberauth/scimply/connector/mysql"
	"github.com/cerberauth/scimply/resource"
	"github.com/cerberauth/scimply/store"
)

func openStore(t *testing.T) *mysqlconn.Store {
	t.Helper()
	...
	s, err := mysqlconn.New(
		mysqlconn.WithDSN(dsn),
		mysqlconn.WithAutoMigrate(true),
		mysqlconn.WithTablePrefix("scimtest_"),
	)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func TestMySQL_Healthy(t *testing.T) {
	s := openStore(t)
...
func TestMySQL_CreateAndGet(t *testing.T) {
	s := openStore(t)
	ctx := context.Background()

	created, err := s.Create(ctx, "user", newUser("alice@example.com"))
	if err != nil {
...
func TestMySQL_GetNotFound(t *testing.T) {
	s := openStore(t)
...
func TestMySQL_CreateConflict(t *testing.T) {
	s := openStore(t)
	ctx := context.Background()

	suffix := randomSuffix()
	userName := "conflict-" + suffix + "@example.com"
...
func TestMySQL_Patch(t *testing.T) {
	s := openStore(t)
	ctx := context.Background()
