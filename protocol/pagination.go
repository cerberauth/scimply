...
func ParseListParamsFromValues(v url.Values) ListParams {
	...
	if sortOrder := strings.ToLower(v.Get("sortOrder")); sortOrder == orderDescending {
		params.SortOrder = orderDescending
	} else if sortOrder == orderAscending {
		params.SortOrder = orderAscending
	}
