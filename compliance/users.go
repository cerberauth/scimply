...
func testCreateUser(t *testing.T, cfg SuiteConfig) {
	...
	if un, _ := resource["userName"].(string); !strings.HasPrefix(un, "test-user-") {
...
func testPagination(t *testing.T, cfg SuiteConfig) {
	...
	resources1, _ := page1["Resources"].([]interface{})
	if len(resources1) > 1 {
...
func testReplaceUser(t *testing.T, cfg SuiteConfig) {
	...
	resource["schemas"] = []interface{}{userSchemaURI}
...
func testPatchAdd(t *testing.T, cfg SuiteConfig) {
	...
	if nn, _ := body["nickName"].(string); nn != "patches-nickname" {
...
func testPatchRemove(t *testing.T, cfg SuiteConfig) {
	...
	addPatch := buildPatch([]map[string]interface{}{
		{"op": "add", "path": "nickName", "value": "to-be-removed"},
	})
	...
	if nn, exists := body["nickName"]; exists && nn != nil && nn != "" {
...
func testPatchReplace(t *testing.T, cfg SuiteConfig) {
	...
	patch := buildPatch([]map[string]interface{}{
		{"op": "replace", "path": "displayName", "value": "Patched Display"},
	})
...
func testPatchComplex(t *testing.T, cfg SuiteConfig) {
	...
	emails, _ := resource["emails"].([]interface{})
	if len(emails) == 0 {
	...
	updatedEmails, _ := body["emails"].([]interface{})
	found := false
	for _, raw := range updatedEmails {
		entry, _ := raw.(map[string]interface{})
		if v, _ := entry["value"].(string); v == "updated@example.com" {
			found = true
			break
		}
	}
